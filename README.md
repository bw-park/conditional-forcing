...
All application and media paths are relative so the site works under a project subdirectory. Video uses HLS with local fMP4 segments, native browser playback when available, and the bundled hls.js fallback. Muted videos autoplay while visible; the gallery loads 12 cards at a time.
